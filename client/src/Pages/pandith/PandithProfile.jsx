...
import { getPandithInfo, updatePandithProfile } from "../../api/api";
import { useNavigate, useParams } from "react-router-dom";
import { Button, Col, Form, Input, InputNumber, Row, TimePicker } from "antd";
import moment from "moment";
import toast from "react-hot-toast";

const PandithProfile = () => {
  const { user } = useSelector((state) => state.user);
  const [pandith, setPandith] = useState(null);

  const dispatch = useDispatch();
  const params = useParams();
  const navigate = useNavigate();

  //Fetch Pandith information
  const fetchPandithInfo = async () => {
    const userId = params.id;
    try {
      dispatch(showLoading());
      const response = await getPandithInfo(userId);
  ...
  const onFinish = async (values) => {
    ...
    try {
      dispatch(showLoading());
      const response = await updatePandithProfile(data);
      dispatch(hideLoading());
      navigate("/");
        ...
        {pandith && (
          <Form
            ...
            initialValues={{
              ...pandith,
              timings: [
                moment(pandith.timings[0], "HH:mm"),
                moment(pandith.timings[1], "HH:mm"),
              ],
            }}
          >
